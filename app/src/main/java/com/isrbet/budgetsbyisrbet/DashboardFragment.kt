...
/*    private fun getBudgetAmount(iBudgetMonth: BudgetMonth, iCategory: String, iSubcategory: String, iActualAmount: Double, iWhoToLookup: String) : Double {
        Log.d("Alex", "getting budget for " + iBudgetMonth.toString())
        ...
        for (i in 0 until SpenderViewModel.getTotalCount()) {
            if (SpenderViewModel.isActive(i)) {
                if (iWhoToLookup == "Joint" || iWhoToLookup == SpenderViewModel.getSpenderName(i) ||
                    SpenderViewModel.getSpenderName(i) == "Joint") {
                    val budgetForPeriod = BudgetViewModel.getCalculatedBudgetAmount(
                        iCategory + "-" + iSubcategory,
                        iBudgetMonth,
                        SpenderViewModel.getSpenderName(i)
                    )
                    Log.d("Alex", "got $iCategory $iSubcategory " + SpenderViewModel.getSpenderName(i) + " " + iBudgetMonth.toString() + " " + budgetForPeriod.dateStarted.toString() + " " + budgetForPeriod.amount)

                    if (iBudgetMonth.month == 0 || budgetForPeriod.dateStarted.month != 0) { // ie an annual view, or not an annual budget
                        if (iWhoToLookup == "Joint" || iWhoToLookup == SpenderViewModel.getSpenderName(i)) { // ie want the entire amounts
                            tBudgetAmount += budgetForPeriod.amount
                        } else {
                            if (iWhoToLookup != "Joint" && SpenderViewModel.getSpenderName(i) == "Joint") { // ie want a specific person, so also need his/her share of the Joint budget
                                tBudgetAmount += budgetForPeriod.amount * splitToUse / 100
                            }
                        }
                    } else {
                        handlingAnnualBudget = true
                        var totalAnnualBudget = BudgetViewModel.getCalculatedBudgetAmount(
                            iCategory + "-" + iSubcategory,
                            iBudgetMonth,
                            SpenderViewModel.getSpenderName(i)
                        ...
                        accumulatedBudget += budgetRemaining
                    }
                }
            }
        }
        if (handlingAnnualBudget) {
            tBudgetAmount =
                if (iActualAmount < accumulatedBudget) iActualAmount else accumulatedBudget
        }
        Log.d("Alex", "returning $iCategory $iSubcategory "  + " " + iBudgetMonth.toString() + " " + tBudgetAmount)
